...
static char *to_lowercase(const char *str) {
    ...
    char *lower = (char *) palloc(len + 1); // Выделение памяти для результата
    for (size_t i = 0; i < len; i++) {
        lower[i] = tolower((unsigned char)str[i]); // Преобразование символа
    }
    lower[len] = '\0';
    return lower;
}
...
static unsigned int hash_function(const char *str) {
    unsigned int hash = 0;
    while (*str) {
        hash = (hash * 31) + (unsigned char)*str++; // Хэширование символов строки
    }
    return hash;
}
...
Datum count_unique_words(PG_FUNCTION_ARGS) {
    ...
    for (token = strtok_r(input_str, " ", &saveptr); token != NULL; token = strtok_r(NULL, " ", &saveptr)) {
        ...
        if (hash_table[index] == 0) {                      
            ...
            unique_count++;
        } else if (hash_table[index] != hash) {             // Хэш-коллизия (редкий случай)
